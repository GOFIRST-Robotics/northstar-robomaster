...
#include "cv_aiming_indicator.hpp"
...
{
CvAimingIndicator::CvAimingIndicator(
    RefSerialTransmitter &refSerialTransmitter,
    const RefSerial &refSerial,
    src::control::governor::CvOnTargetGovernor &cvAimingGovorner)
    : HudIndicator(refSerialTransmitter),
      refSerial(refSerial),
      cvAimingGovorner(cvAimingGovorner)
{
}

modm::ResumableResult<void> CvAimingIndicator::update()
{
    if (cvAimingGovorner.isReady())
    {
...
modm::ResumableResult<void> CvAimingIndicator::sendInitialGraphics()
{
    RF_BEGIN(0);
...
void CvAimingIndicator::initialize()
{
